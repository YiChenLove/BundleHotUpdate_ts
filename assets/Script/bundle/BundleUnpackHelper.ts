...
export default class BundleUnpackHelper extends SingleIns<BundleUnpackHelper>() {
    ...
    execUnpackage(onComplete) {
        ...
        if (
            GlobalConst.Client_Version == localClientVer &&
            jsb.fileUtils.isFileExist(path_cache + 'cacheList.json')
        ) {
            ...
            let coverCachelist = () => {
                ...
                // 覆盖本地资源版本号
                BundleUtil.LOG(
                    CodeType.BundleUnpackHelper,
                    'abVersion__:',
                    JSON.stringify(abVersion)
                );
                ...
                cacheMag.writeCacheFile(() => {
                    ...
                    cc.sys.localStorage.setItem(
                        ModuleConst.localVersionConfigKey2,
                        JSON.stringify(abVersion)
                    );
                    cc.sys.localStorage.setItem(
                        ModuleConst.localClientVer,
                        GlobalConst.Client_Version
                    );
                    onComplete();
                });
