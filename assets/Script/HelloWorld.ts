...
export default class SubGame_2 extends cc.Component {
    ...
    onLoad() {
        ...
        if (typeof jsb != 'undefined') {
            let absPath1 = jsb.fileUtils
                .fullPathForFilename(this.asset1.nativeUrl)
                .replace('//', '/');
            let absPath2 = jsb.fileUtils
                .fullPathForFilename(this.asset2.nativeUrl)
                .replace('//', '/');
        ...
        BundleManager.inst.execUnpackage(() => {
            BundleManager.inst.reqVersionInfo(() => {
                // 获取最新版本
    ...
    reloadLobbyRoot() {
        ...
        BundleManager.inst.hotUpdateMultiModule(loadAb, () => {
            // 更新模块到最新版本

            BundleManager.inst.addModule('ABLobby', (moduleObj) => {
                // 加载模块

                let abObj = moduleObj.getABObj();

                abObj.load('Scene/LobbyRoot', cc.Prefab, (err, prefab) => {
                    // 使用模块资源
    ...
    hackSysLog() {
        ...
        let push_log = function (...arg) {
            ...
            for (let idx = 0; idx < strLen; ) {
                ...
                if (!ignore) {
                    _logArr.push(
                        '_' + _logArr.length + '_=> ' + splitStr + (endIdx < strLen ? '-->' : '')
                    );
                }
        ...
        let consoleLogDef = function (...arg) {
            ...
            if (!ignore) {
                if (cc['_sv_console_log_2_Ori']) {
                    cc['_sv_console_log_2_Ori'].call(console, ...arg);
                }
            }
        };
        if (!cc['_sv_log_2_Ori']) {
